...
│ ╚═╝╚═╝╩═╝        by Zalan Demeter │                                                                  
    ...
    public:
        ...
        virtual size_t size(uuid id) const = 0;
    ...
    public:
        ...
        {
            ALGORITHM<AdjacencyList<DATA_TYPE>> algorithm;
            algorithm.template traverse<policy>(*this, id, [](const Vertex<DATA_TYPE> &vertex)
    ...
    public:
        base_const_iterator cbegin() const override { return base_const_iterator(std::make_shared<const_iterator<>>(m_vertices.cbegin())); }
        base_const_iterator cend() const override { return base_const_iterator(std::make_shared<const_iterator<>>(m_vertices.cend())); }
        base_iterator begin() override { return base_iterator(std::make_shared<iterator<>>(m_vertices.begin())); }
        base_iterator end() override { return base_iterator(std::make_shared<iterator<>>(m_vertices.end())); }

        base_const_iterator cbegin(uuid id) const override { return base_const_iterator(
            std::make_shared<const_iterator<const_neighbor_iterator>>(m_vertices.at(id).second.cbegin())); }
        base_const_iterator cend(uuid id) const override { return base_const_iterator(
            std::make_shared<const_iterator<const_neighbor_iterator>>(m_vertices.at(id).second.cend())); }
        base_iterator begin(uuid id) override { return base_iterator(
            std::make_shared<iterator<neighbor_iterator>>(m_vertices.at(id).second.begin())); }
        base_iterator end(uuid id) override { return base_iterator(
    ...
    private:
        ...
        public:
            ...
            const VERTEX_TYPE &operator*() const override { return *m_it->second.first; }
            const VERTEX_TYPE *operator->() const override { return m_it->second.first.get(); }
        ...
        public:
            ...
            const VERTEX_TYPE &operator*() const override { return **m_it; }
            const VERTEX_TYPE *operator->() const override { return m_it->get(); }
        ...
        public:
            ...
            VERTEX_TYPE &operator*() const override { return *m_it->second.first; }
            VERTEX_TYPE *operator->() const override { return m_it->second.first.get(); }
        ...
        public:
            ...
            VERTEX_TYPE &operator*() const override { return **m_it; }
            VERTEX_TYPE *operator->() const override { return m_it->get(); }
    ...
    public:
        ...
        uuid add_vertex([[maybe_unused]] const VERTEX_TYPE &&vertex) override = 0;
        uuid add_vertex([[maybe_unused]] const DATA_TYPE &&data) = 0;
        void add_edge([[maybe_unused]] uuid vertex1, [[maybe_unused]] uuid vertex2) = 0;

        void remove_vertex([[maybe_unused]] uuid vertex) = 0;
        void remove_edge([[maybe_unused]] uuid vertex1, [[maybe_unused]] uuid vertex2) = 0;

        const VERTEX_TYPE &get_vertex([[maybe_unused]] uuid id) const = 0;
        VERTEX_TYPE &get_vertex([[maybe_unused]] uuid id) = 0;

        size_t size() const = 0;
        size_t size([[maybe_unused]] uuid id) const = 0;
        void empty() = 0;

        std::ostream &print([[maybe_unused]] std::ostream &os = std::cout) const = 0;
    ...
    public:
        base_const_iterator cbegin() const override = 0;
        base_const_iterator cend() const override = 0;
        base_iterator begin() override = 0;
        base_iterator end() override = 0;

        base_const_iterator cbegin(uuid id) const override = 0;
        base_const_iterator cend(uuid id) const override = 0;
        base_iterator begin(uuid id) override = 0;
        base_iterator end(uuid id) override = 0;
    };
    ...
    template <typename DATA_TYPE>
    auto add = std::bind([](Vertex<DATA_TYPE> &vertex, DATA_TYPE a)
                         { vertex.get_data() = vertex.get_data() + a; },
                         std::placeholders::_1, std::placeholders::_2);

    template <typename DATA_TYPE>
    auto increment = std::bind([](Vertex<DATA_TYPE> &vertex, DATA_TYPE a)
                               { vertex.get_data() = vertex.get_data() + a; },
                               std::placeholders::_1, 1);

    template <typename DATA_TYPE>
    auto decrement = std::bind([](Vertex<DATA_TYPE> &vertex, DATA_TYPE a)
                               { vertex.get_data() = vertex.get_data() + a; },
                               std::placeholders::_1, -1);

    template <typename DATA_TYPE>
    auto multiply = std::bind([](Vertex<DATA_TYPE> &vertex, DATA_TYPE a)
                              { vertex.get_data() = vertex.get_data() * a; },
                              std::placeholders::_1, std::placeholders::_2);

    template <typename DATA_TYPE, VertexFormat format = VertexFormat::SHORTEST, std::ostream &os = std::cout>
    auto print = std::bind([](const Vertex<DATA_TYPE> &vertex)
                           { os << format << vertex << std::endl; },
                           std::placeholders::_1);

    template <typename DATA_TYPE>
    auto less_than = std::bind([](const Vertex<DATA_TYPE> &vertex, DATA_TYPE a)
                               { return vertex.get_data() < a; },
                               std::placeholders::_1, std::placeholders::_2);

    template <typename DATA_TYPE>
    auto greater_than = std::bind([](const Vertex<DATA_TYPE> &vertex, DATA_TYPE a)
                                  { return vertex.get_data() > a; },
                                  std::placeholders::_1, std::placeholders::_2);

    template <typename DATA_TYPE>
    auto equal_to = std::bind([](const Vertex<DATA_TYPE> &vertex, DATA_TYPE a)
                              { return vertex.get_data() == a; },
                              std::placeholders::_1, std::placeholders::_2);
